...
public class ChannelManager {
    ...
    public static void createRestrictedChannel(Guild server,List<Member> members, String name){
        ...
        TextChannel channel = server.createTextChannel(name).addMemberPermissionOverride(server.getJDA().getSelfUser().getIdLong(),Permission.MANAGE_PERMISSIONS.getRawValue(),0).addRolePermissionOverride(server.getPublicRole().getIdLong(),0,Permission.ALL_PERMISSIONS).complete();
        TextChannelManager m =channel.getManager();
        for(Member member : members){
            m.putMemberPermissionOverride(member.getIdLong(), Permission.ALL_TEXT_PERMISSIONS,Permission.MESSAGE_ATTACH_FILES.getRawValue()).queue();
        }
    }
