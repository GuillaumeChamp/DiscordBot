...
public class Vote implements ParallelAction{
    ...
    public void handleAction(Member voter, Member target, String action) throws ProcessingException {
        ...
        if (RoleManagement.isNotIn(roles, target)) throw new ProcessingException(target.getEffectiveName()+" this player is not in the game");
        if (RoleManagement.isNotIn(roles, voter)) throw new ProcessingException(voter.getEffectiveName()+" : You seem to not be in the game");
        if (!RoleManagement.isA(roles,voter,this.rolesToVote)) throw new ProcessingException("It's not your tour to vote");
        for(int i = 0; i< roles.size(); i++){
    ...
    public ArrayList<Role> getResult() throws ProcessingException{
        ...
        if (first==null) throw new ProcessingException("The village was quiet and no one wanted to vote for some one\nIt's so sad, it's always funny to see someone burn !");
