...
class WaiterTest {
    @Test
    void shouldWaiterAwakeGameProperly() throws InterruptedException {
        Member testMember = DiscordUtil.getOwner();
        Game game = new Game(0, Collections.singletonList(testMember),null);
    ...
    @Test
    void shouldWaiterHoldFewActions() throws InterruptedException {
        Member testMember = DiscordUtil.getOwner();
        Game game = new Game(0, Collections.singletonList(testMember),null);
        Game game1 = new Game(0, Collections.singletonList(testMember),null);
        Role role = new Role(testMember, EnhanceRoleType.simpleVillager);
        Vote vote = new Vote(VoteType.all,Collections.singletonList(role));
        Vote vote1 = new Vote(VoteType.werewolf,Collections.singletonList(role));
        Waiter.register(game,vote);
        Waiter.register(game1,vote1);
        Awaitility.await().atLeast(59, TimeUnit.SECONDS).atMost(62, TimeUnit.SECONDS).until(()->!vote.isActive()&&!vote1.isActive());
    }
